...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CBitcoinRandomAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
{
    CBitcoinRandomAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid()) {
    ...
    if (!(CBitcoinRandomAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
