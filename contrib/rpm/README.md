...
The RPM spec file provided here is for BitcoinRandom-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinrandom.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitcoinRandom git repository](https://github.com/bitcoinrandom/bitcoinrandom) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinrandom.conf` file.
...
in the bitcoinrandom git repository. At present only an SVG version of the BitcoinRandom
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinrandom
...
needed when 0.12.1 is released, a proper fix is already in the BitcoinRandom git
master branch.
...
idea to build BitcoinRandom Core against a static version of that library compiled
...
The `bitcoinrandom.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinrandom-server` package. At this point, most RPM based
...
The `bitcoinrandom-server` package should have SELinux support. How to properly do
...
BitcoinRandom-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinrandom/). He has been
packaging BitcoinRandom for Fedora at least since 2012.
...
4. I renamed the `bitcoinrandom` package that contains the Qt GUI to `bitcoinrandom-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinrandom-xt` or `bitcoinrandom-classic`. I wanted to make sure the general population
