...
###   bitcoinrandomcore.org and bitcoinrandom.org and compares them.
...
WORKINGDIR="/tmp/bitcoinrandom_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinrandomcore.org"
HOST2="https://bitcoinrandom.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinrandom-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinrandom-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinrandom.org failed to provide signature file, but bitcoinrandomcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinrandom.org and bitcoinrandomcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinRandom Core binary release signing key installed?"
   fi
